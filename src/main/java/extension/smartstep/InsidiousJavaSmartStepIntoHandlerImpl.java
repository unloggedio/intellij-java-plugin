...
import extension.util.LocationUtil;
...
public class InsidiousJavaSmartStepIntoHandlerImpl extends InsidiousJavaSmartStepIntoHandler {
    private static final Logger LOG = Logger.getInstance(InsidiousJavaSmartStepIntoHandler.class);
    ...
    private Promise<List<SmartStepTarget>> findSmartStepTargetsAsync(SourcePosition position, InsidiousJavaDebugProcess debugProcess, boolean smart) {
        ...
        res.setResult(
                ReadAction.compute(() -> {
                    ...
                    return findStepTargets(position, debugProcess, (XSuspendContext) suspendContext, stackFrameProxy, smart);
                }));

        return (Promise<List<SmartStepTarget>>) res;
    }
                ...
                private ThreeState evaluateCondition(@Nullable PsiElement condition) {
                    if (condition != null && !DebuggerUtils.hasSideEffects(condition)) {
                        ...
                        try {
                            ...
                            ExpressionEvaluator evaluator = EvaluatorBuilderImpl.getInstance().build(condition, position, (EvaluationContext) evaluationContextImpl);
                            return ThreeState.fromBoolean(
                                    EvaluatorUtil.evaluateBoolean(evaluator, (EvaluationContext) evaluationContextImpl));
                        } catch (EvaluateException e) {
                            InsidiousJavaSmartStepIntoHandlerImpl.LOG.info((Throwable) e);
                        }
                    }
                ...
                if (stackFrameProxy != null) {
                    ...
                    try {
                        ...
                        visitLinesInstructions(stackFrameProxy
                                .location(), true, lines, (opcode, owner, name, desc, itf, ordinal) -> removeMatchingMethod(methodTargets, owner, name, desc, ordinal, (InsidiousVirtualMachineProxy) debugProcess.getConnector()));
                ...
                while (iterator.hasNext()) {
                    ...
                    if (e instanceof MethodSmartStepTarget &&
                            DebuggerUtil.methodMatches(((MethodSmartStepTarget) e)
                                    .getMethod(), owner
                                    .replace("/", "."), name, desc, (InsidiousVirtualMachineProxy) this.myDebugProcess
