...
public class InsidiousJavaValue extends XNamedValue implements InsidiousNodeDescriptorProvider {
    ...
    public void computePresentation(@NotNull final XValueNode node, @NotNull final XValuePlace place) {
        ...
        this.myValueDescriptor.updateRepresentationNoNotify(xDebugProcess, new DescriptorLabelListener() {
                    ...
                    if (text.length() > 1000) {
                        node.setFullValueEvaluator(new JavaFullValueEvaluator(InsidiousJavaValue.this.myEvaluationContext) {
                            public void evaluate(@NotNull final XFullValueEvaluator.XFullValueEvaluationCallback callback) {
                                ...
                                fullValueDescriptor.updateRepresentation(this.myEvaluationContext.getVirtualMachineProxy().getXDebugProcess(), () -> callback.evaluated(fullValueDescriptor.getValueText()));
                            }
                        });
    ...
    public Promise<XExpression> calculateEvaluationExpression() {
        ...
        try {
            getDescriptor().getTreeEvaluation(this).whenComplete((psiExpression, ex) -> {
                if (ex != null) {
                    result.setError(ex);
        ...
        return new XInstanceEvaluator() {
            ...
            public void evaluate(@NotNull XDebuggerEvaluator.XEvaluationCallback callback, @NotNull XStackFrame frame) {
                ...
                if (InsidiousJavaValue.this.myEvaluationContext != null) {
                    callback.evaluated(InsidiousJavaValue.create(inspectDescriptor, InsidiousJavaValue.this.myEvaluationContext, InsidiousJavaValue.this.myNodeManager));
                } else {
